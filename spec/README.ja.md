...
言語の定義中心は`SEP`と`END_SEP`です。同時に開いている構造識別子は同じ名前空間に
属し、一意でなければなりません。閉じたラベルは再利用できます。
...
release互換性とversioning方針は[Versioningと互換性方針](versioning.ja.md)を参照してください。
...
厳密な統計、進数変換、区切り付き2／8／16進数literalを定義します。短い数学名は
互換aliasとして残します。
...
- [Embedded board mapping](embedded-board-mapping.ja.md): 論理pin、review済みTier 1
  board profile、capability-aware bus検証、host adapter境界、Pico／Pico 2のSDK firmware
  生成とdeployを実装しています。
