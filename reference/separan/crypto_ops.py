...
def _key(value, function, position, runtime):
    ...
    if len(raw) != KEY_LENGTH:
        raise error("E920", "crypto_error", f"{function}() requires an exact 32-byte AES-256 key.", position, expected="32 bytes", actual=f"{len(raw)} bytes")
    return raw
...
def _restore(kind, value, position):
    ...
    if kind == TEXT_KIND:
        ...
        except UnicodeDecodeError: raise error("E921", "crypto_error", "Authenticated container contains invalid UTF-8 text.", position)
    raise error("E921", "crypto_error", "Authenticated container has an unsupported payload type.", position)


def _bounded(value, function, position):
    if len(value) > MAX_CRYPTO_BYTES:
        raise error("E920", "crypto_error", f"{function}() input exceeds the cryptographic size limit.", position, expected=f"at most {MAX_CRYPTO_BYTES} bytes", actual=f"{len(value)} bytes")
...
def _decrypt_authenticated(arguments, named, position, runtime):
    ...
    if len(raw) < minimum or len(raw) > MAX_CRYPTO_BYTES + minimum or not raw.startswith(AEAD_MAGIC):
        raise error("E921", "crypto_error", "Authenticated container is malformed or too large.", position)
    ...
    if version != CONTAINER_VERSION or kind not in (TEXT_KIND, BYTES_KIND, SECRET_KIND):
        raise error("E921", "crypto_error", "Authenticated container version or payload type is unsupported.", position)
    offset += 2
    ...
    except InvalidTag: raise error("E922", "crypto_authentication_error", "Ciphertext was modified or the key is incorrect.", position)
...
def _derive_key_from_password(arguments, named, position, runtime):
    ...
    if len(salt.value) < SALT_LENGTH or len(salt.value) > 1024:
        raise error("E923", "crypto_error", "Argon2id salt must contain between 16 and 1024 bytes.", position, expected="16..1024 bytes", actual=f"{len(salt.value)} bytes")
...
def _decrypt_with_password(arguments, named, position, runtime):
    ...
    if len(raw) < fixed + TAG_LENGTH or len(raw) > MAX_CRYPTO_BYTES + fixed + TAG_LENGTH or not raw.startswith(PASSWORD_AEAD_MAGIC):
        raise error("E921", "crypto_error", "Password-encrypted container is malformed or too large.", position)
    ...
    if version != CONTAINER_VERSION or kind not in (TEXT_KIND, BYTES_KIND, SECRET_KIND) or (memory, time_cost, lanes) != (ARGON_MEMORY_KIB, ARGON_TIME, ARGON_LANES):
        raise error("E921", "crypto_error", "Password-encrypted container version or Argon2id parameters are unsupported.", position)
    ...
    except InvalidTag: raise error("E922", "crypto_authentication_error", "Ciphertext was modified or the password is incorrect.", position)
    return _restore(kind, plaintext, position)
